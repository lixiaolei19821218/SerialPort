...
namespace Monitor.ViewModel
{
    class MainWindowVM : ViewModelBase
    {
        ...
        {
            if (shiftSignalPort.IsOpen)
            {
                shiftSignalPort.Close();
            }
            if (qrCodePort.IsOpen)
            {
                qrCodePort.Close();
            }
        ...
        private void SendToDB2()
        {
            ...
            var barcodes = repo.BarCodes.ToList().Where(b => b.DateTime.HasValue && b.DateTime.Value > DateTime.Parse("2016-08-09 17:00"));
